...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('DOM elements found:', {
    ...
    streamContents: streamContents.length,
  ...
  async function loadMeritList(showLoader = true) {
    try {
      ...
      const hasData = Object.values(meritData).some(streamData => streamData && streamData.length > 0);
      
      if (!hasData) {
        console.log('No data found, showing empty state');
        showEmptyState();
      } else {
        console.log('Data found, rendering streams');
        renderAllStreams();
        showCurrentStream();
      }
  ...
  function renderStreamList(stream, tbody) {
    console.log(`Rendering ${stream} stream with`, tbody ? 'valid' : 'invalid', 'tbody');
    
    if (!tbody) {
      console.error(`No tbody found for ${stream} stream`);
      return;
    }
    ...
    console.log(`${stream} stream has ${students.length} students`);
  ...
  function showCurrentStream() {
    console.log('Showing current stream:', currentStream);
    ...
    if (currentContent) {
      ...
      console.log('Made active:', currentContent.id);
    } else {
      console.error('Could not find content for stream:', currentStream);
    }
